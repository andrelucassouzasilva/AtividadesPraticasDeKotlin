...
### 6. Documentando o próprio processo de versionamento

Em seguida, esta própria seção "Controle de versionamento" foi escrita e commitada, para deixar registrado o passo a passo e os comandos usados:

```bash
git add README.md
git commit -m "Documenta o processo de versionamento e comandos Git utilizados"
git push
```

### 7. Edição direta pelo GitHub e sincronização do repositório local

Depois desse commit, o `README.md` foi editado diretamente pela interface web do GitHub (removendo temporariamente a lista de atividades). Como essa alteração foi feita direto no repositório remoto, o clone local ficou desatualizado. Para trazer essa mudança para a máquina local, foi usado:
...
O `git fetch` busca os commits novos do remoto sem alterar os arquivos locais, e o `git status` mostrou que a branch local estava "atrasada" em relação à `origin/main` (podendo ser atualizada com fast-forward):

```bash
git pull origin main
```

O `git pull` trouxe o commit feito pelo GitHub para o repositório local, atualizando o `README.md` na máquina.

### 8. Restaurando o conteúdo completo do README

Para recuperar o conteúdo completo (documentação das atividades + versionamento) removido na edição pelo GitHub, foi usado o conteúdo do `README.md` de um commit anterior, através de `git show`:

```bash
git show 74329b3:README.md > README.md
git status --short
git add README.md
git commit -m "Restaura documentacao completa do README (atividades e versionamento)"
git push
```

`git show <commit>:<arquivo>` exibe (ou, redirecionado com `>`, salva) o conteúdo de um arquivo exatamente como ele estava em um commit específico, sem precisar reverter o histórico inteiro.
...
| `git status` / `git status --short` | Verifica o que será commitado antes de confirmar |
...
| `git fetch origin` | Busca commits novos do remoto sem alterar os arquivos locais |
| `git pull origin main` | Busca e já aplica (merge/fast-forward) os commits novos do remoto |
| `git show <commit>:<arquivo>` | Mostra/recupera o conteúdo de um arquivo em um commit específico |
